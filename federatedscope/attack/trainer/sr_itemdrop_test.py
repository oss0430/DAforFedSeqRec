...
def wrap_SrDropTest(
    ...
    ## NOT AN Actual attack, but uses attack trainer
    base_trainer.ctx.itemdrop_method = \
        base_trainer.cfg.srtest.itemdrop_method
    
    base_trainer.ctx.number_of_drops = \
        base_trainer.cfg.srtest.number_of_drops
    
    # ---- action-level plug-in -------
    base_trainer.replace_hook_in_train(
        new_hook = hook_on_batch_forward_dropped_sequence,
...
def hook_on_batch_forward_dropped_sequence(ctx):
